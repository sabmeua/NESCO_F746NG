...
namespace nesco::device
{
    ...
    bool FilesystemDiscoF746NG::seek(NescoFileHandle h, uint32_t offset, uint8_t origin)
    {
        FILE *fp = static_cast<FILE *>(h);
    ...
    bool FilesystemDiscoF746NG::readLine(NescoFileHandle h, char *buff)
    {
        FILE *fp = static_cast<FILE *>(h);
    ...
    bool FilesystemDiscoF746NG::writeLine(NescoFileHandle h, const char *buff)
    {
        FILE *fp = static_cast<FILE *>(h);
        fputs(buff, fp);
    ...
    void FilesystemDiscoF746NG::close(NescoFileHandle h)
    {
        FILE *fp = static_cast<FILE *>(h);
        fclose(fp);
    }
